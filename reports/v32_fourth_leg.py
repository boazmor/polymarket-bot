#!/usr/bin/env python3
"""Test the user's hypothesis: keep ask 0.60 (cheap entries), then add the 15m
part-3 as a FOURTH confirming leg. Does it keep cheap prices AND lift win%/dollars?
Base = NEW config (3-of-4 fast @0.60, no time limit, buy cheapest tradeable).
Then partition each trade by whether the concurrent 15m part-3 (predict/lim/okx)
AGREES at the fire moment. Compare win%, avg price, net per-trade (2% comm)."""
import csv, statistics
from collections import defaultdict, Counter

THR=0.60; INVEST=2.0; COMM=0.02; GAP=200; SMIN=10; SMAX=295
...
def lim_outs():
    ...
    return {ep:('UP' if lb[mid]>t else 'DOWN') for mid,ep in m.items() if (mid in lb and mid in tg) for t in [tg[mid]]}
def load_allsec(path,epc,sc,up,dn,tg):
    ...
    return out
def load_lim_allsec(path,mk,up,dn,tg,off):
    m={}
...
def main():
    ...
    lim=load_lim_allsec(LIM,LIMMK,'best_ask','no_best_ask','target_price',300)
    gem=load_allsec(GEM,'market_open_epoch','sec_from_open','best_ask','no_best_ask','target_price')
    kal=load_kal_byes()
    # 15m part-3 sources keyed by 15m-open; for 5m window O the 15m open = O-600
    ...
    l15=load_lim_allsec(LIM15,LIM15MK,'best_ask','no_best_ask','target_price',900)

    def p15vote(ep,sec):
        o=ep-600; tsec=sec+600
        votes=[]
        for src in (p15,o15,l15):
            w=src.get(o)
            if not w: continue
            # nearest sec within 20
            best=None;bd=999
            for s in w:
                if abs(s-tsec)<bd: bd=abs(s-tsec);best=s
            if best is not None and bd<=20: votes.append(vote(w[best]))
        return votes
    ...
    for ep in eps:
        ...
        for sec in range(SMIN,SMAX+1):
            ...
            for side in ('UP','DOWN'):
                ...
                pv=p15vote(ep,sec)
                p15_agree=sum(1 for v in pv if v==side)
                p15_opp=sum(1 for v in pv if v and v!=side)
                trades.append((side,plat,price,oc==side,p15_agree,p15_opp,len(pv)))
                fired=True;break
            if fired: break
    ...
    def rep(rows,label):
        ...
        avgp=statistics.mean([r[2] for r in rows])
        ...
        be=avgp*(1+COMM)*100
        print(f'  {label:<40} n={n:<4} win%={100*w/n:5.1f} avgP={avgp:.3f} net=${net:+7.2f} per=${per:+.3f} sd=${sd:.2f} BE={be:.1f}')

    print('=== base: 3-of-4 fast @0.60, no time limit, buy cheapest tradeable ===')
    rep(trades,'ALL (base)')
    print()
    print('=== add 4th leg: 15m part-3 ===')
    rep([r for r in trades if r[6]>0],'has any 15m part-3 data')
    rep([r for r in trades if r[4]>=1 and r[5]==0],'15m part-3 AGREE (>=1, 0 oppose)')
    rep([r for r in trades if r[4]>=2 and r[5]==0],'15m part-3 AGREE >=2')
    rep([r for r in trades if r[5]>=1],'15m part-3 has an OPPOSER')
    rep([r for r in trades if r[6]==0],'no 15m part-3 data')
