#!/usr/bin/env python3
"""MIDDLE third (part 2) of the 15-min window, mirroring the part-3 analysis.
...
# PART 2: 15m open = ep5 - 300 ; read at sec_from_open = 5m_sec + 300
def aligned(ep):
    o=ep-300; return (o in p15) or (o in o15) or (o in l15)
def leg15(ep,sec,side):
    o=ep-300;ts=sec+300;conf=[]
...
for ep in eps:
    ...
    for sec in range(SMIN,SMAX+1):
        ...
        for side in ('UP','DOWN'):
            ...
            t15=p15s.get(ep-300); t5=p5s.get(ep)
...
print('MIDDLE third (part 2) of 15m, $%g/trade, span %.2f days'%(INVEST,span))
print('='*74)
...
rep(base,'part-2 + consensus')
rep(d20,'+ dist>=20')
rep(leg4,'+ 15m part-2 confirms')
...
if sims: print('  median 15m-5m target gap: $%.1f (part-3 was ~$55-72)'%statistics.median(sims))
