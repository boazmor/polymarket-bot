#!/usr/bin/env python3
"""arb_v6_3way_live.py - 3-platform live arbitrage bot for 1-HOUR markets.

Monitors Polymarket + Predict.fun + Limitless 1-hour BTC up/down markets.
Same engine as arb_v5_3way_live.py but with:
  - 1h window math (now // 3600 * 3600)
  - Polymarket slug pattern bitcoin-up-or-down-{month}-{day}-{year}-{hour12}{am/pm}-et
  - 1h data file paths

All 6 candidates supported: A_POLY, B_POLY, A_LIM, B_LIM, LimUP_PolyDN, PolyUP_LimDN.

Usage:
    python3 -u arb_v6_3way_live.py --max-trades-per-window 2 --invest 7.0
"""
