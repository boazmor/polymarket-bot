...
MAX_VIRTUAL_BUY_USD = 100.0
DATA_DIR = "data_5m_dual"
...
ANSI_DIM = "\033[2m"
ANSI_BLINK = "\033[5m"
MIN_DIST_BOT120 = 68.0
BOT120_PRICE_CAP = 0.71
BOT40_MIN_FLOW_DISTANCE = 20.0
BOT40_ANALYSIS_DISTANCE_LEVELS = [float(x) for x in range(25, 69)]
BOT40_ANALYSIS_PRICE_LEVELS = [round(x / 100.0, 2) for x in range(30, 36)]
BOT40_LIMIT_PRICE = 0.31
BOT40_LIMIT_END_SEC = 30
BOT40_FALLBACK_PRICE = 0.35
RENDER_RETRY_WINDOW_SEC = 20
RENDER_RETRY_INTERVAL_SEC = 2
BOT40_FLOW_DIST_THRESHOLD = 20.0
BOT40_RESEARCH_PRICE_LEVELS = [round(x / 100.0, 2) for x in range(30, 43)]
BOT40_RESEARCH_SECONDS = list(range(30, 41))
BOT120_RESEARCH_SECONDS = list(range(30, 121))
BOT120_RESEARCH_DISTANCE_LEVELS = [float(x) for x in range(64, 73)]
...
class DualResearchLogger:
    ...
    def clear_and_init(self) -> None:
        ...
        self.paths = {
            ...
            "parameter_matrix": os.path.join(self.data_dir, "parameter_matrix.csv"),
            "sanity_summary": os.path.join(self.data_dir, "sanity_summary.csv"),
            "bot40_30_40_matrix": os.path.join(self.data_dir, "bot40_30_40_matrix.csv"),
        }
        headers = {
            ...
            "trade_outcomes": ["entry_ts", "settle_ts", "bot", "slug", "sec_from_start", "buy_side", "winner_side", "result", "spent_usd", "qty", "avg_fill_price", "payout_total", "pnl_total", "entry_btc_price", "entry_target_price", "distance_signed", "flow_side", "with_flow", "entry_best_ask", "entry_best_bid", "settle_btc_price", "settle_target_price"],
            "parameter_matrix": ["ts", "bot", "slug", "sec_from_start", "buy_side", "result", "pnl_total", "avg_fill_price", "distance_signed", "flow_side", "with_flow", "test_type", "distance_threshold", "price_cap", "would_include"],
            "sanity_summary": ["ts", "slug", "bot", "virtual_buys_total", "settled_markets", "wins", "losses", "pushes", "realized_pnl_total", "note"],
            "bot40_30_40_matrix": ["ts", "slug", "sec_from_start", "price_level", "distance_level", "btc_price", "target_price", "distance_signed", "flow_side", "flow_best_ask", "flow_best_bid", "flow_qty_le_level", "flow_notional_le_level", "would_trigger_with_flow", "note"],
        }
        ...
        self._init_csv(self.paths["parameter_matrix"], headers["parameter_matrix"])
        self._init_csv(self.paths["sanity_summary"], headers["sanity_summary"])
        self._init_csv(self.paths["bot40_30_40_matrix"], headers["bot40_30_40_matrix"])
        with open(os.path.join(self.data_dir, "BOT_DOCUMENTATION_V2.txt"), "w", encoding="utf-8") as f:
            f.write("GOOD_WORKING_BOT_KULULU_V2\n")
            f.write("Changes: BOT120 dist>=68 price<=0.71; BOT40 flow-only dist>=20; 30-40 kept and scanned for dist 25-68 with flow; clearer screen; trade_outcomes and parameter_matrix reports.\n")
    ...
    def log_bot40_30_40_matrix(self, slug: str, sec: int, price_level: float, distance_level: float,
                               btc_price: Optional[float], target_price: Optional[float], distance: Optional[float],
                               flow_side: Optional[str], flow_best_ask: Optional[float], flow_best_bid: Optional[float],
                               flow_qty: Optional[float], flow_notional: Optional[float], would_trigger: int, note: str) -> None:
        self._append_csv(self.paths["bot40_30_40_matrix"], [
            self._ts(), slug, sec, price_level, distance_level, btc_price, target_price, distance,
            flow_side, flow_best_ask, flow_best_bid, flow_qty, flow_notional, would_trigger, note,
        ])

    def log_trade_outcome(self, bot: BotState, slug: str, pos: VirtualPosition, winner_side: str,
                          result: str, payout: float, pnl: float, settle_btc_price: Optional[float],
                          settle_target_price: Optional[float]) -> None:
        self._append_csv(self.paths["trade_outcomes"], [
            pos.entry_ts or self._ts(), self._ts(), bot.name, slug, pos.sec, pos.side, winner_side, result,
            round(pos.spent, 6), round(pos.qty, 6), round(pos.avg_fill, 6), round(payout, 6), round(pnl, 6),
            pos.entry_btc_price, pos.entry_target_price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow,
            pos.entry_best_ask, pos.entry_best_bid, settle_btc_price, settle_target_price,
        ])
        self.log_parameter_matrix_rows(bot, slug, pos, result, pnl)

    def log_parameter_matrix_rows(self, bot: BotState, slug: str, pos: VirtualPosition, result: str, pnl: float) -> None:
        if pos.entry_distance is None or pos.avg_fill is None:
            return
        distance_abs = abs(float(pos.entry_distance))
        price = float(pos.avg_fill)
        rows = []
        if bot.name == "BOT40":
            for d in range(16, 25):
                include = int(distance_abs >= d and bool(pos.entry_with_flow))
                rows.append([self._ts(), bot.name, slug, pos.sec, pos.side, result, round(pnl, 6), price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow, "BOT40_DIST_FLOW", float(d), "", include])
            if 30 <= pos.sec <= 40:
                for d in range(25, 69):
                    for cap in BOT40_ANALYSIS_PRICE_LEVELS:
                        include = int(distance_abs >= d and price <= cap and bool(pos.entry_with_flow))
                        rows.append([self._ts(), bot.name, slug, pos.sec, pos.side, result, round(pnl, 6), price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow, "BOT40_30_40_DIST_PRICE_FLOW", float(d), cap, include])
        elif bot.name == "BOT120":
            for d in range(64, 73):
                rows.append([self._ts(), bot.name, slug, pos.sec, pos.side, result, round(pnl, 6), price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow, "BOT120_DIST_ONLY", float(d), "", int(distance_abs >= d)])
            for cap_i in range(67, 76):
                cap = round(cap_i / 100.0, 2)
                rows.append([self._ts(), bot.name, slug, pos.sec, pos.side, result, round(pnl, 6), price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow, "BOT120_PRICE_ONLY", "", cap, int(price <= cap)])
            for d in range(64, 73):
                for cap_i in range(67, 76):
                    cap = round(cap_i / 100.0, 2)
                    rows.append([self._ts(), bot.name, slug, pos.sec, pos.side, result, round(pnl, 6), price, pos.entry_distance, pos.entry_flow_side, pos.entry_with_flow, "BOT120_DIST_PRICE", float(d), cap, int(distance_abs >= d and price <= cap)])
        for row in rows:
            self._append_csv(self.paths["parameter_matrix"], row)

    def log_sanity(self, slug: str, bot: BotState, note: str) -> None:
        self._append_csv(self.paths["sanity_summary"], [
            self._ts(), slug, bot.name, bot.virtual_buy_count, bot.settled_markets, bot.wins, bot.losses, bot.pushes, round(bot.realized_pnl_total, 6), note,
        ])
...
class Polymarket5mDualBot:
    ...
    async def _capture_rendered_page_target(self) -> None:
        ...
        rendered_target, source, err = await self.extract_target_from_rendered_page(self.current.get("url") or "")
    ...
    def _bot40_level_and_mode(self, sec: int) -> Tuple[Optional[float], Optional[str]]:
        ...
        if sec <= BOT40_LIMIT_END_SEC:
            return BOT40_LIMIT_PRICE, "LIMIT_031"
    ...
    def _choose_bot40_side(self, sec: int) -> Optional[str]:
        ...
        if btc is None or target is None or distance is None or flow_side is None:
            return None
        if abs(distance) < BOT40_MIN_FLOW_DISTANCE:
            return None
        if self.bot40.last_buy and self.bot40.current_market_spent < MAX_VIRTUAL_BUY_USD:
            existing_side = self.bot40.last_buy["side"]
            if existing_side != flow_side:
                return None
        ...
        ask = self.prices[flow_side]["best_ask"]
        if ask is None or ask > level or not self._side_has_fresh_ask_for_current_market(flow_side):
            return None
        return flow_side
    ...
    def _choose_bot120_side(self, sec: int) -> Optional[str]:
        ...
        if ask is None or ask > BOT120_PRICE_CAP or not self._side_has_fresh_ask_for_current_market(direction_side):
            return None
    ...
    def _record_bot40_30_40_matrix_for_second(self, sec: int) -> None:
        if not (30 <= sec <= 40):
            return
        btc, target, distance = self._distance_fields()
        flow_side = self._flow_side_from_distance(distance)
        if flow_side is None:
            for price_level in BOT40_ANALYSIS_PRICE_LEVELS:
                for distance_level in BOT40_ANALYSIS_DISTANCE_LEVELS:
                    self.logger.log_bot40_30_40_matrix(self.current["slug"], sec, price_level, distance_level, btc, target, distance, None, None, None, None, None, 0, "no_flow_side")
            return
        flow_best_ask = self.prices[flow_side]["best_ask"]
        flow_best_bid = self.prices[flow_side]["best_bid"]
        for price_level in BOT40_ANALYSIS_PRICE_LEVELS:
            flow_qty, flow_notional = self._qty_notional_le(flow_side, price_level)
            for distance_level in BOT40_ANALYSIS_DISTANCE_LEVELS:
                would_trigger = int(btc is not None and target is not None and distance is not None and abs(distance) >= distance_level and flow_best_ask is not None and flow_best_ask <= price_level)
                self.logger.log_bot40_30_40_matrix(self.current["slug"], sec, price_level, distance_level, btc, target, distance, flow_side, flow_best_ask, flow_best_bid, flow_qty, flow_notional, would_trigger, "flow_only_scan")
    ...
    def _try_execute_bot_buy(self, bot: BotState, sec: int, side: Optional[str]) -> None:
        ...
        if side is None:
            ...
            if bot is self.bot40:
                ...
                if mode == "LIMIT_031":
                    bot.last_note = f"no side at or below {BOT40_LIMIT_PRICE:.2f}"
                else:
        ...
        if bot.current_market_spent >= MAX_VIRTUAL_BUY_USD - 1e-9:
        ...
        if bot is self.bot40:
            ...
            mode = "MARKET_CAP_071"
            price_limit = BOT120_PRICE_CAP
            if best_ask is None or best_ask > price_limit:
                bot.last_decision = "WAIT"
                bot.last_note = f"{side} ask missing or above {price_limit:.2f}"
                return
        ...
        remaining_cap = max(0.0, MAX_VIRTUAL_BUY_USD - bot.current_market_spent)
        ...
        pos = VirtualPosition(
            ...
            entry_with_flow=int(side == self._flow_side_from_distance(distance)) if distance is not None else None,
        ...
        bot.buy_done_for_market = bot.current_market_spent >= MAX_VIRTUAL_BUY_USD - 1e-9
        bot.triggers_seen += 1
        ...
        for side_name in ["UP", "DOWN"]:
            for pos in bot.positions[side_name]:
                ...
                if pos_pnl > 1e-9:
                    ...
                    pos_result = "PUSH"
                self.logger.log_trade_outcome(bot, slug, pos, winner_side, pos_result, pos_payout, pos_pnl, btc, target)
        self.logger.log_sanity(slug, bot, "settled_market")
    ...
    def print_status(self) -> None:
        ...
        print(f"{ANSI_BOLD}GOOD_WORKING_BOT_KULULU_V2{ANSI_RESET}")
        print("=" * width)
        print(f"LOCAL TIME   : {self.now_local_str()}")
        print(f"SLUG         : {self.current['slug'] or '-'}")
        sec_now = self.seconds_from_market_start()
        time_left = max(0, 300 - sec_now)
        print(f"SEC FROM STRT: {sec_now} | TIME LEFT: {time_left}s")
        print(f"BOT40 RULE   : 0-30 <= {BOT40_LIMIT_PRICE:.2f}, 31-40 <= {BOT40_FALLBACK_PRICE:.2f}, FLOW ONLY, DIST>={BOT40_MIN_FLOW_DISTANCE:.0f}")
        print(f"BOT40 30-40 RESEARCH: FLOW ONLY scan DIST 25-68 and PRICE 0.30-0.35")
        print(f"BOT120 RULE  : {BOT120_MIN_SEC}-{BOT120_MAX_SEC}s FLOW ONLY, DIST>={MIN_DIST_BOT120:.0f}, PRICE<={BOT120_PRICE_CAP:.2f}, USD={MAX_VIRTUAL_BUY_USD:.0f}")
        print(f"BINANCE BTC  : {self.fmt(btc_price, 2)} | updated: {updated}")
        print(f"TARGET USED  : {self.fmt(target_used, 2)} | TGT RENDERED: {self.fmt(self.current.get('target_rendered_page'), 2)} | TARGET STATUS: {self.current.get('render_retry_status') or '-'}")
        signed_dist = None if btc_price is None or target_used is None else (btc_price - target_used)
        flow_side = self._flow_side_from_distance(signed_dist)
        print(f"DIST SIGNED  : {self.fmt(signed_dist, 2)} | FLOW SIDE: {flow_side or '-'} | BOT120 MIN DIST: {MIN_DIST_BOT120:.2f}")
        print("-" * width)
        ...
        print(f"SANITY: BOT40_BUYS={self.bot40.virtual_buy_count} BOT40_MKTS={self.bot40.settled_markets} | BOT120_BUYS={self.bot120.virtual_buy_count} BOT120_MKTS={self.bot120.settled_markets}")
                    ...
                    try:
                        while True:
                            ...
                            if sec != last_logged_sec:
                                ...
                                self._record_bot40_30_40_matrix_for_second(sec)
