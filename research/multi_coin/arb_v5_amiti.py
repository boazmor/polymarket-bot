#!/usr/bin/env python3
"""
arb_v5_amiti.py — V5 AMITI ("real" V5) — same as V5_4seasons but applies
the two filters my analysis recommended on 07/05:

  FILTER 1: spread of all 4 strikes must be in [$20, $40]
  FILTER 2: Gemini must NOT be the highest of the 4 strikes

Per analysis on 192 V5 trades:
  Without filters: 31% wins, -$1,069
  With these 2 filters: 73% wins, +$957 on 57 trades

Same trading pair as V5: Poly + Predict.fun only.

arb_v5_poly_predict_4seasons.py origin — Polymarket + Predict.fun bot with
...
Output: /root/arb_v5_amiti_trades.csv

Run:
  screen -dmS arb_v5_amiti python3 /root/arb_v5_amiti.py
"""
import csv
import os
import subprocess
