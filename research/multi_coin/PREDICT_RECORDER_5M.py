...
Run:
    python3 PREDICT_RECORDER_5M_V2.py --data-dir /root/data_predict_btc_5m
"""
...
DATA_DIR = "/root/data_predict_btc_5m"
...
def slug_for_15m_market_open(market_open_epoch: int) -> str:
    """Predict.fun 15m slug uses same pattern as Polymarket: btc-updown-5m-{epoch}"""
    return f"btc-updown-5m-{market_open_epoch}"


def current_15m_market_open() -> int:
    """Floor current time to nearest 15-min boundary (UTC)."""
    return (now_epoch() // 300) * 300
...
async def main():
    ...
    csvs.event("START", f"PREDICT_RECORDER_5M_V2 starting; data_dir={DATA_DIR}")
