...
Run:
    python3 PREDICT_RECORDER_1H_V2.py --data-dir /root/data_predict_eth_1h
"""
...
DATA_DIR = "/root/data_predict_eth_1h"
...
def slug_for_market_open(market_open_epoch: int) -> str:
    """Compute Predict.fun slug for a market that opens at the given UTC epoch.
    Slug format observed: ethereum-up-or-down-{month}-{day}-{year}-{hour}{am|pm}-et
    ...
    return f"ethereum-up-or-down-{month_tag}-{et_dt.day}-{et_dt.year}-{hour_tag}-et"


def current_hour_market_open() -> int:
...
async def market_manager(csvs: CsvStore):
    ...
    while not SHOULD_STOP:
        try:
            target_open = current_hour_market_open()
            if CURRENT_MARKET_OPEN_EPOCH != target_open:
                ...
                # Resolve new market
                slug = slug_for_market_open(target_open)
...
async def main():
    ...
    csvs.event("START", f"PREDICT_RECORDER_1H_V2 starting; data_dir={DATA_DIR}")
