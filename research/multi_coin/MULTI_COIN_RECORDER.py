...
async def poly_loop(state: RecorderState, csvs: CsvStore) -> None:
    """Reconnects on every market rollover. Long-lived WS connections silently
    stop receiving book updates after a few minutes (observed: poly_book_ticks
    froze 3 min after recorder start despite SUBSCRIBE re-sent on each market).
    Home LIVE bot uses one-WS-per-market and gets continuous data."""
    last_sub_key = None
    while not state.should_stop:
        try:
