...
DEFAULT_SIMILAR_GAP = 200 # trio (5m) targets must be within this many USD
DEFAULT_DIST_MIN = 20     # mandatory check 1: |binance-target| >= this (avoid small-distance flip zone)
DEFAULT_TGTSIM_MAX = 60   # mandatory check 2: |15m_target - 5m_target| < this
...
# ----- output files -----
OUT_DECISIONS = "live5_decisions.csv"
OUT_TRADES = "live5_trades.csv"
OUT_LIMITLESS = "live5_limitless.csv"
...
def decide_live5(poly, pred, lim, gem, okx, s15, strike15,
                 thr, similar_gap, dist_min, tgtsim_max):
    """LIVE5 = part-3 recipe (called only on part-3-aligned windows).
    1. >=3 of {pred,lim,okx,gem} (5m) agree on a side at ask>=thr, trio targets within similar_gap.
    2. distance |pred binance-strike| >= dist_min (mandatory check 1).
    3. >=1 of {pred15,okx15,lim15} part-3 agrees with the side (4th leg).
    4. |15m strike - 5m strike| < tgtsim_max (mandatory check 2).
    Buy cheapest of tradeable {poly,pred,lim}.
    Returns (side, platform, price, n_legs, reason, meta) or (None,...reason).
    """
    def gs(snap, side):
        if snap is None: return None
        return (snap.get('up_ask') or snap.get('yes_ask')) if side == 'UP' \
            else (snap.get('down_ask') or snap.get('no_ask'))
    fast = {'pred': pred, 'lim': lim, 'okx': okx, 'gem': gem}
    for side in ('UP', 'DOWN'):
        agree = []
        for nm, snap in fast.items():
            ask = gs(snap, side); tgt = (snap or {}).get('target')
            if ask is None or tgt is None: continue
            if ask >= thr: agree.append((nm, tgt))
        if len(agree) < 3:
            continue
        tgts = [t for _, t in agree]
        if max(tgts) - min(tgts) > similar_gap:
            return None, None, None, 0, 'trio_target_spread', None
        # check 1: distance
        dist = (pred or {}).get('distance')
        if dist is None or abs(dist) < dist_min:
            return None, None, None, 0, f'dist_below_{dist_min}', None
        # 4th leg: 15m part-3 confirmation
        n15 = sum(1 for v in (s15 or {}).values() if v == side)
        if n15 < 1:
            return None, None, None, 0, 'no_15m_confirm', None
        # check 2: 15m-vs-5m target similarity
        strike5 = (pred or {}).get('target')
        if strike15 is None or strike5 is None:
            return None, None, None, 0, 'no_target_sim_data', None
        sim = abs(strike15 - strike5)
        if sim >= tgtsim_max:
            return None, None, None, 0, f'tgtsim_{sim:.0f}', None
        # buy cheapest tradeable
        cands = []
        for p, snap in (('poly', poly), ('pred', pred), ('lim', lim)):
            ask = gs(snap, side)
            if ask is not None and 0.01 < ask < 0.99:
                cands.append((p, ask))
        if not cands:
            return None, None, None, 0, 'no_tradeable_price', None
        plat, price = min(cands, key=lambda x: x[1])
        n_legs = len(agree) + n15
        reason = (f'live5 5m={len(agree)}({"+".join(n for n,_ in agree)}) '
                  f'15m={n15} dist={abs(dist):.0f} tgtsim={sim:.0f} buy={plat}')
        meta = {'n5': len(agree), 'n15': n15, 'dist': abs(dist), 'tgtsim': sim,
                'names': '+'.join(n for n, _ in agree)}
        return side, plat, price, n_legs, reason, meta
    return None, None, None, 0, 'no_3of4_5m_consensus', None
...
def snapshot_okx5(window_epoch, ref_sec, win_half):
    """OKX Event Contracts 5-min snapshot. up_ask/down_ask/target/binance/distance."""
    sec_lo, sec_hi = ref_sec - win_half, ref_sec + win_half
    ts_lo = window_epoch + sec_lo
    rows = tail_rows_since(OKX5_DATA, ts_lo - 5)
    ua, da, tg, bn, ds = [], [], [], [], []
    for r in rows:
        try:
            if int(r.get("market_open_epoch") or 0) != window_epoch:
        ...
        try:
            ...
            continue
    return {"up_ask": median(ua), "down_ask": median(da),
            "target": median(tg), "binance": median(bn),
            "distance": median(ds), "n_samples": len(ua)}


def get_pred15_strike(window_epoch):
    """Base strike of the concurrent 15-min Predict window (open = window_epoch-600).
    The strike is constant across the window, so take it from any matching recent row.
    Used for the 15m-vs-5m target-similarity check (mandatory check 2)."""
    o15 = window_epoch - 600
    rows = tail_rows_since(PRED15_DATA, window_epoch)  # part-3 rows are recent
    strike = None
    for r in rows:
        try:
            if int(r.get("market_open_epoch") or 0) != o15:
                continue
        except (ValueError, TypeError):
            continue
        v = _fnum(r.get("strike"))
        if v is not None:
            strike = v
    return strike
...
def main():
    ...
    p.add_argument("--similar-gap", type=float, default=DEFAULT_SIMILAR_GAP,
                   help="5m trio targets must be within this many USD")
    p.add_argument("--dist-min", type=float, default=DEFAULT_DIST_MIN,
                   help="mandatory check 1: |binance-target| must be >= this")
    p.add_argument("--tgtsim-max", type=float, default=DEFAULT_TGTSIM_MAX,
                   help="mandatory check 2: |15m strike - 5m strike| must be < this")
    ...
    while True:
        try:
            ...
            # LIVE5 trades ONLY part-3-aligned 5m windows (the last third of a 15m window).
            # A 5m window opens at O+600 where O is a 15m boundary -> window_epoch % 900 == 600.
            if window_epoch % 900 != 600:
            ...
            okx = snapshot_okx5(window_epoch, cur_ref, args.win_half)
            ...
            # 15m part-3 confirmation votes + 15m base strike (computed before decide)
            s15 = snapshot_15m_part3(window_epoch, sec_now, args.thr)
            strike15 = get_pred15_strike(window_epoch)
            side, platform, price, n_votes, reason, third = decide_live5(
                poly, pred, lim, gem, okx, s15, strike15,
                args.thr, args.similar_gap, args.dist_min, args.tgtsim_max
            )
            ...
            with open(decisions_path, "a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    kal.get("sub_pos"),
                    consensus, side, platform, price,
                    third["names"] if third else None,
                    strike15,
                    third["tgtsim"] if third else None,
                    reason
                ])
