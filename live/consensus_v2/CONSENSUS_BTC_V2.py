#!/usr/bin/env python3
"""CONSENSUS_BTC_V2 ג€” Poly+Predict consensus bot (BTC 5-min).
...
MODES
  ...
  --live: places real orders via py_clob_client_v2 / PredictTrader.
          ...
          NOT IMPLEMENTED IN THIS REVISION ג€” flip on after dry-run proves out.
...
def decide_v2(poly, pred, lim, gem, kal, thr, window_epoch):
    """V2 logic: require Poly AND Predict BOTH vote same direction.
    Apply distance + NYC hour filters. Other platforms only observed.

    Returns (side, platform, price, n_votes, reason) or (None, None, None, 0, reason_for_skip).
    """
    ...
    # require poly+pred both vote same way
    ...
    if poly_up and pred_up:
        ...
        return None, None, None, 0, "no_poly_pred_consensus"
    # distance filter (poly's distance)
    ...
    # count total agreeing platforms for logging (informational)
    n_agree = 2
    for snap_y, snap_n in [
        ((lim or {}).get("yes_ask"), (lim or {}).get("no_ask")),
        ((gem or {}).get("yes_ask"), (gem or {}).get("no_ask")),
        ((kal or {}).get("yes_ask"), (kal or {}).get("no_ask")),
    ]:
        if side == "UP" and snap_y is not None and snap_y >= thr:
            n_agree += 1
        if side == "DOWN" and snap_n is not None and snap_n >= thr:
            n_agree += 1
    dist_str = f"{dist:.0f}" if dist is not None else "NA"
    return side, plat, price, n_agree, f"v2 poly+pred agree dist={dist_str} nyc={nyc_hr} n_total={n_agree}/5"
...
def main():
    ...
    log(f"CONSENSUS_BTC_V2 starting (DRY-RUN)")
    ...
    while True:
        try:
            ...
            side, platform, price, n_votes, reason = decide_v2(
                poly, pred, lim, gem, kal, args.thr, window_epoch
            )
