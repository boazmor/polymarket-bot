#!/usr/bin/env python3
"""CONSENSUS_BTC_V3 — Poly+Predict consensus + third-platform confirmation (BTC 5-min).
...
FILES WRITTEN (relative to script dir or --out-dir)
  consensus_v3_decisions.csv  one row per evaluated sec
  consensus_v3_trades.csv     one row per simulated/live trade
  consensus_v3_outcomes.csv   one row per window's resolution
  consensus_v3.log            stdout (run with python3 -u for live tailing)
...
# ----- output files -----
OUT_DECISIONS = "consensus_v3_decisions.csv"
OUT_TRADES = "consensus_v3_trades.csv"
OUT_LIMITLESS = "consensus_v3_limitless.csv"
OUT_GEMINI = "consensus_v3_gemini.csv"
OUT_KALSHI = "consensus_v3_kalshi.csv"
OUT_OUTCOMES = "consensus_v3_outcomes.csv"
...
def decide_v3(poly, pred, lim, gem, kal, thr, similar_gap, window_epoch):
    """V3 logic: require Poly AND Predict BOTH vote same direction,
    distance filter on Poly (skip if in [50,100]),
    AND require at least one of (Lim, Kal) to:
      - have target within similar_gap of avg(poly_target, pred_target)
      - vote same direction >= thr

    Returns (side, platform, price, n_votes, reason, third_meta)
    or (None, None, None, 0, reason_for_skip, None).
    """
