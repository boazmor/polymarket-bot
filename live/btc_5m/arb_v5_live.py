            ...
            if poly_ok and not pred_ok:
                ...
                if cap_price > 0.10:
                    try:
                        ...
                        log_order("POLY_RETRY", response=json.dumps(retry_resp, default=str)[:200],
                                  status=(retry_resp or {}).get("status"), filled=retry_filled)
                    except Exception as e:
                        log_order("POLY_RETRY_ERROR", err=f"{type(e).__name__}: {e}")
                if not retry_filled:
                    # CRITICAL: cancel the retry order before selling predict, else it may fill later and create unhedged exposure
                    try:
                        if retry_resp and retry_resp.get("orderID"):
                            cancel_resp = poly_client.cancel_orders([str(retry_resp["orderID"])])
                    ...
                    try:
                        ...
                        log_order("PREDICT_SELL_RESULT", code=sell_resp.get("code"),
                                  orderId=sell_resp.get("orderId"))
                    except Exception as e:
                        log_order("PREDICT_SELL_ERROR", err=f"{type(e).__name__}: {e}")
                window_has_unhedged = True
                log_order("WINDOW_BLOCKED", reason="predict_unwind", window=current_epoch)
