#!/usr/bin/env python3
"""arb_v9_dip_buy_5m.py — Dip buy strategy on 5-min BTC markets.

Strategy:
  Watch the FIRST 40 seconds of every 5-min window across all 3 platforms.
  If at any moment a side's best_ask drops to <= BUY_THRESH (0.35), BUY
  that side on the platform offering it.

  Hold the position. Continuously watch the best_bid of the same side.
  When bid >= buy_price * (1 + PROFIT_PCT) (i.e. 40% profit), SELL.

  If never reaches profit target, hold to expiry. Position worth $1 if
  the side wins, $0 if it loses.

No freshness checks. Single buy per window. Dry-run by default.
"""
...
# Strategy parameters
BUY_THRESH = 0.35     # buy if any side's best_ask <= this
PROFIT_PCT = 0.40     # sell when bid >= buy_price * (1 + this)
BUY_WINDOW_SEC = 40   # only buy during first N seconds of market
...
def find_dip(snap):
    """Find the lowest ask <= BUY_THRESH across all platforms and both sides.
    Returns (plat, side, ask_price, ask_depth_usd) or None."""
    best = None
    for plat, s in snap.items():
        ...
        for side in ("yes", "no"):
            a = s[f"{side}_ask"]
            d = s[f"{side}_ask_usd"]
            if a is None or a <= 0 or a > BUY_THRESH:
                continue
...
def main():
    ...
    tag = f"5m_${int(args.invest)}"
    orders_path = f"/root/arb_v9_dip_{tag}_orders.csv"
    log_order = make_log_fn(orders_path)

    print(f"=== arb_v9_dip_buy_5m starting at {now_iso()} ===", flush=True)
    print(f"invest=${args.invest}  dry_run={args.dry_run}  "
          f"buy<={BUY_THRESH}  profit={int(PROFIT_PCT*100)}%  "
          f"buy_window={BUY_WINDOW_SEC}s", flush=True)
    ...
    while True:
        try:
            ...
            if window_epoch != current_epoch:
                # If we had an open position from previous window, force-sell or
                # accept settlement.
                if position:
            ...
            # If position held, watch for sell
            if position:
                snap = read_state_snapshot()
                bid = snap[position["plat"]][f"{position['side']}_bid"]
                if bid > 0:
                    profit_pct = (bid - position["buy_price"]) / position["buy_price"]
                    if profit_pct >= PROFIT_PCT:
                        log_order("SELL_TRIGGER",
                                  plat=position["plat"], side=position["side"],
                                  buy_price=round(position["buy_price"], 4),
                                  cur_bid=round(bid, 4),
                                  profit_pct=round(profit_pct * 100, 1))
                        target_price = bid
                        res = sell_leg(ctx, position["plat"], position["side"],
                                       position["shares"], log_order, target_price)
                        log_order("SELL_RESULT",
                                  plat=position["plat"], ok=res.get("ok"),
            ...
            # No position. Only buy in first BUY_WINDOW_SEC.
            if sec_in_window > BUY_WINDOW_SEC:
                time.sleep(POLL)
                continue
            ...
            plat, side, ask_price, ask_depth = dip
